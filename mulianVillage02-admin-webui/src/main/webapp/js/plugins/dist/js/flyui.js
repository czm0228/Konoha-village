...
(function e(t, n, r) {
    ...
    2: [
        function(require, module, exports) {
            ...
            var tmpl = template.compile(
                '<div class="alert alert-{{css}}" role="alert"><i></i><span>{{content}}</span></div>'
            );
            ...
            var Alert = Widget.extend({
                ...
                __check: function() {
                    container.children('.alert:gt(' + (maxLength - 1) + ')').remove();
                },

                __animate: function() {
                    this.element.hide().show(300);
                    /*var element = this.element.addClass('fade').css({
            marginLeft: -this.element.width() / 2 - 1
        });
        element.animate({
            top: 0
        }, 300, function() {
            element.removeClass('fade').css({
                marginLeft: 'auto'
            });
        });*/
                },
    ...
    3: [
        function(require, module, exports) {
            ...
            "g:\\frontend\\flyui\\.build\\temp\\fly.accordion.js": 1
        }
    ],
    4: [
        function(require, module, exports) {
            ...
            var Calender = Widget.extend({
                ...
                ctor: function(element, options) {
                    ...
                    if (that.options.value) {
                        date = fly.parseDate(that.options.value);
                    }
                ...
                _wrapper: function() {
                    ...
                    each(modes, function(i, item) {
                        ...
                        that._createBody(item).appendTo(that[item])
                            .on(CLICK, 'td.' + item.substring(0, item.length - 1),
                                proxy(
                                    that['_select' + item.substring(0, 1).toUpperCase() +
                                        item.substring(1, item.length - 1)], that));
                ...
                _createHeader: function(mode) {
                    var that = this,
                        date = that.date,
                        arrow = (mode == 'days' || mode == 'years') ?
                        '<i class="next glyphicon glyphicon-menu-right"></i>' +
                        '<i class="prev glyphicon glyphicon-menu-left"></i>' : '',
                        title, html, header;

                    if (mode == 'days') {
                        title = '<span class="year">' + date.getFullYear() +
                            '年</span><span class="month">' + months[date.getMonth()] +
                            '</span>';
                    } else {
                        title = '<div class="date">请选择' + modeMapping[mode] + '</div>';
                    }

                    html = '<div class="switch clearfix">' + arrow +
                ...
                _createBody: function(mode) {
                    ...
                    if (mode == 'days') {
                        thead = '<thead><tr>';
                        thead += map(days, function(value) {
                            return '<th class="dow">' + value + '</th>';
                        }).join('');
                        thead +=
                            '</tr><tr><th colspan="7" class="border"></th></tr></thead>';
                    }
                    return $('<table cellpadding="0" cellspacing="0">' + thead +
                        '<tbody></tbody></table>');
                },
                ...
                _createFooter: function(mode) {
                    var that = this,
                        date = that.date,
                        html = '';
                    if (mode == 'days' && that.time) {
                        html += '<div class="control clearfix">';
                        html +=
                            '<button type="button" class="ok btn btn-primary btn-xs">确定</button>';
                        html +=
                            '<button type="button" class="cancel btn btn-xs">取消</button>';
                        html += '<div class="time">';
                        html += '<span class="hour">' + that._parseTime(date.getHours()) +
                            '</span>:';
                        html += '<span class="minute">' + that._parseTime(date.getMinutes()) +
                            '</span>:';
                        html += '<span class="second">' + that._parseTime(date.getSeconds()) +
                            '</span>';
                        html += '</div>';
                        html += '</div>';
                        return $(html).on(CLICK, '.ok', proxy(that._ok, that))
                            .on(CLICK, '.cancel', proxy(that._cancel, that))
                ...
                _fillDays: function() {
                    ...
                    while (prevMonth.valueOf() < nextMonth) {
                        ...
                        if ((prevMonth.valueOf() + 86400000) <= options.startDate) {
                            clsName += ' disabled';
                        }
                        if (prevMonth.valueOf() > options.endDate) {
                            clsName += ' disabled';
                        }
                ...
                _fillTimes: function(column, total, mode) {
                    var viewDate = this.viewDate,
                        ...
                        i = 0;
                    if (mode == 'hours') {
                        viewTime = viewDate.getHours();
                    } else if (mode == 'minutes') {
                        viewTime = viewDate.getMinutes();
                    } else if (mode == 'seconds') {
                        viewTime = viewDate.getSeconds();
                    }
                    for (; i < total; i++) {
                        if (i % column == 0) {
                            row = $('<tr>');
                        ...
                        row.append('<td class="' + item + ' ' + (i === viewTime ? (' ' +
                                    ACTIVE) :
                                '') +
                            '">' + this._parseTime(i) + '</td>');
                    }
                    return rows;
                },
                ...
                _switch: function(mode) {
                    ...
                    each(modes, function(i, item) {
                        if (mode == item) {
                            that[item].show();
                            that['_fill' + item.substring(0, 1).toUpperCase() +
                                item.substring(
                                    1, item.length)]();
                        } else {
                            that[item].hide();
                ...
                _ok: function(e) {
                    ...
                    this.trigger('ok', {
                        event: e,
                        date: newDate,
                        result: fly.parseDate(newDate, this.options.format)
                    });
                },
        ...
        function(require, module, exports) {
            ...
            var ComboBox = Select.extend({
                ...
                ctor: function(element, options) {
                    ...
                    options.placeholder = options.placeholder || element.attr(
                        "placeholder");
                ...
                _input: function() {
                    ...
                    if (!input[0]) {
                        wrapper.append(
                            '<span tabindex="-1" unselectable="on" class="dropdown-wrap state-default">' +
                            '<input ' + name +
                            'class="input" type="text" autocomplete="off"/>' +
                            '<span tabindex="-1" unselectable="on" class="select">' +
                            '<i class="glyphicon glyphicon-triangle-bottom"></i></span></span>'
                        )
                            .append(that.element);
                    ...
                    that._arrow = wrapper.find(".glyphicon")
                        .attr({
                            "role": "button",
                            "tabIndex": -1
                        });
        ...
        function(require, module, exports) {
            ...
            var DatePicker = Widget.extend({
                ...
                _wrapper: function() {
                    ...
                    if (!span[0]) {
                        wrapper.append(
                            '<span unselectable="on" class="datepicker-wrap ' + DEFAULT +
                            '">' +
                ...
                _calender: function() {
                    ...
                    that.calender = new Calender(that.calenderWrapper, extend({}, this.options, {
                        ...
                        cancel: proxy(that._cancelHandler, that)
                    }));
                },
    ...
    15: [
        function(require, module, exports) {
            ...
            var _isFixed = !fly.ie6 && !fly.support.touch;
            ...
            var innerHTML =
                '<div i="dialog" class="dialog">' + '<div class="dialog-arrow-a"></div>' +
                '<div class="dialog-arrow-b"></div>' + '<table class="dialog-grid">' + '<tr>' +
                '<td i="header" class="dialog-header">' +
                '<button i="close" class="dialog-close">&#215;</button>' +
                '<div i="title" class="dialog-title"></div>' + '</td>' + '</tr>' + '<tr>' +
                '<td i="body" class="dialog-body">' +
                '<div i="content" class="dialog-content"></div>' +
                '</td>' + '</tr>' + '<tr>' + '<td i="footer" class="dialog-footer">' +
                '<div i="statusbar" class="dialog-statusbar"></div>' +
                '<div i="button" class="dialog-button"></div>' + '</td>' + '</tr>' + '</table>' +
                '</div>';
        ...
        function(require, module, exports) {
            ...
            var DropDownList = Select.extend({
                ...
                _allowOpening: function(length) {
                    return this.optionLabel[0] || this.filterInput || this.dataSource.view()
                        .length;
                },
                ...
                _optionLabel: function() {
                    ...
                    if (!template) {
                        ...
                        if (typeof optionLabel === "string") {
                            ...
                            template += fly.expr(options.dataTextField, "data");
                        }
                ...
                _selectNext: function(index) {
                    ...
                    for (; index < length; index++) {
                        ...
                        if (text && that._matchText(text, index) && !(that._word.length ===
                            1 &&
                            startIndex === that.selectedIndex)) {
                            return true;
                        }
                    }
                ...
                _keypress: function(e) {
                    ...
                    if (e.which === 0 || e.keyCode === keys.ENTER) {
                        return;
                    }
                ...
                _focusElement: function(element) {
                    ...
                    var compareElement = element === filterInput ? wrapper :
                        filterInput;
                ...
                _lastItem: function() {
                    this.optionLabel.removeClass(FOCUSED);
                    this.listView.last();
                },

                _nextItem: function() {
                    if (this.optionLabel.hasClass(FOCUSED)) {
                        this.optionLabel.removeClass(FOCUSED);
                        this.listView.first();
                    } else {
                        this.listView.next();
                ...
                _prevItem: function() {
                    ...
                    if (!this.listView.focus()) {
                        this.optionLabel.addClass(FOCUSED);
                    }
                },
                ...
                _span: function() {
                    ...
                    if (!span[0]) {
                        wrapper.append(
                            '<span unselectable="on" class="dropdown-wrap ' + DEFAULT +
                            '">' +
                            '<span unselectable="on" class="input">&nbsp;</span>' +
                            '<span unselectable="on" class="select">' +
            ...
            var Form = Widget.extend({
                ...
                serialization: function(isValidate) {
                    var $form = this.element,
                        ...
                        $items = $form.find(
                            'input:enabled[name], textarea:enabled[name]'),
        ...
        function(require, module, exports) {
            ...
            var Gird = Widget.extend({
                ...
                loadData: function() {
                    ...
                    this.autoAdd();
                    self.options.afterRender&&self.options.afterRender();
                    // this.sum(data);
                    // this.initFCol();
                }
            })
    ...
    25: [
        function(require, module, exports) {
            ...
            var Pagination = Widget.extend({
                ...
                _wrapper: function() {
                    var that = this,
                        pageControl,currentPageNum,showPerPage;
                    that.element.addClass('pagination clearfix');
                    that.numberView = $('<div class="number clearfix" />').appendTo(
                        that.element);
                    pageControl = $('<div class="control" />').appendTo(that.element);
                    // currentPageNum=$('<div class="control" />').appendTo(that.element);
                    // showPerPage=$('<div class="control" />').appendTo(that.element);
                    that.totalView = $('<div class="info" />').appendTo(pageControl);
                    // that.totalView = $('<div class="info" />').appendTo(showPerPage);
                    that.switchView = $('<div class="switch" />').appendTo(pageControl);
                    // that.switchView = $('<div class="switch" />').appendTo(showPerPage);
                ...
                _getInterval: function(currentPage) {
                    ...
                    var ne_half = math.floor(numDisplayEntries / 2),
                ...
                _createLink: function(page_id, current_page, appendopts) {
                    ...
                    if (page_id == current_page) {
                        lnk = $('<span class="btn btn-primary btn-sm current">' +
                            appendopts.text + '</span>');
                    } else {
                        lnk = $('<a class="btn btn-default btn-sm">' + appendopts.text +
                            '</a>')
                ...
                _appendRange: function(current_page, start, end, opts) {
                    var i = start;
                    for (; i < end; i++) {
                        this._createLink(i, current_page, opts).appendTo(this.numberView);
                    }
                },
                ...
                _drawLinks: function(current_page) {
                    ...
                    if (opts.showNumber) {
                        if (interval.start > 1 && numEdgeEntries > 0) {
                            end = math.min(numEdgeEntries, interval.start);
                            that._appendRange(current_page, 1, end, {
                                classes: 'sp'
                            });
                            if (numEdgeEntries < interval.start && opts.ellipseText) {
                                $("<span>" + opts.ellipseText + "</span>").appendTo(
                                    numberView);
                            }
                        }
                        ...
                        if (interval.end <= np && numEdgeEntries > 0) {
                            if (np - numEdgeEntries > interval.end && opts.ellipseText) {
                                $("<span>" + opts.ellipseText + "</span>").appendTo(
                                    numberView);
                            }
                            begin = math.max(np - numEdgeEntries, interval.end);
                            that._appendRange(current_page, begin, np + 1, {
                                classes: 'ep'
                            });
                        }
                    }
...
// haha
                ...
                _drawInfo: function() {
                    ...
                    if (this.options.showTotal && dataSource.options.total !== false) {
                        this.totalView.html('共' + dataSource.total() + '条记录，当前第'+ dataSource.page()+'/'+
                            dataSource.totalPages() + '页每页显示'+dataSource.pageSize()+'条');
